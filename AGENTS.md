...
Guidance for AI coding agents working on the Varbase AI Figma Base recipe.
...
- **Merge request comments and history** on
  [git.drupalcode.org/project/varbase_ai_figma_base](https://git.drupalcode.org/project/varbase_ai_figma_base/-/merge_requests)
  — the reasoning behind recent changes lives in the MR discussions.
- **Issue comments** on
  [drupal.org/project/issues/varbase_ai_figma_base](https://www.drupal.org/project/issues/varbase_ai_figma_base)
...
- This is a `drupal-recipe`; keep configuration in the recipe's own `recipe.yml`
  and `config/`, not in a `config/install` or `config/optional` module directory.
